...
import { getCurriculumMastery } from "../progression.js";
import { evaluateMeaningfulAchievements, MEANINGFUL_ACHIEVEMENTS, MILESTONE_DEFINITIONS } from "../achievements.js";

const $ = (selector, root = document) => root.querySelector(selector);
const escapeHtml = (value = "") => String(value).replace(/[&<>\"]/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[char]);

function masteryData(state) {
  return getCurriculumMastery(state).levels.map((level) => ({ ...level, unlocked: state.unlockedLevels.includes(level.id) || level.id === 1 }));
}
...
function categoryLabel(category) { return { mastery: "Mastery", exploration: "Exploration", excellence: "Excellence" }[category] || category; }
...
function ensureStyles() {
  ...
  const style = document.createElement("style"); style.id = "mastery-styles";
...
function renderAll() {
  ...
  const { state: nextState, mastery } = syncAchievements(state);
  renderCompact(nextState, mastery);
  renderProgress(nextState, mastery);
}
