...
function levelCard(state, levelIndex, currentLevel) {
  ...
  return `<article class="journey-level-card journey-level-card--${kind}">
    <span class="journey-level-icon" aria-hidden="true">${icon}</span>
    <div class="journey-level-copy"><span>LEVEL ${levelIndex + 1}</span><strong>${esc(title)}</strong><small>${esc(description)}</small></div>
    <div class="journey-level-progress"><strong>${p.completed}/${p.total}</strong><small>${kind === "locked" ? "Locked" : `${p.percent}%`}</small></div>
  </article>`;
}
...
function render() {
  ...
  mount.innerHTML = `
    <section class="journey-status" aria-label="Learning progress">
      <div><span class="journey-kicker">YOUR JOURNEY</span><h1>Keep going.</h1><p>Level ${currentLevel + 1} · ${esc(meta[0])}</p></div>
      <div class="journey-metrics"><span><strong>${state.xp.toLocaleString()}</strong> XP</span><span><strong>${state.streak}</strong> day streak</span><span><strong>${total.percent}%</strong> overall</span></div>
    </section>

    <section class="current-mission" aria-labelledby="current-mission-title">
      <div class="current-mission-main">
        <div class="current-mission-meta"><span>CONTINUE LEARNING</span><span>LEVEL ${currentLevel + 1} · MISSION ${current.lessonIndex + 1} OF ${p.total}</span></div>
        <div class="current-mission-heading"><div><span>${esc(meta[0])}</span><h2 id="current-mission-title">${esc(current.lesson.title)}</h2></div><strong>+${Number(current.lesson.xp) || 0} XP</strong></div>
        <p>${esc(current.lesson.description)}</p>
        <div class="current-mission-progress"><div><span>Level progress</span><strong>${p.completed} / ${p.total}</strong></div><div class="journey-progress-track" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${p.percent}" aria-label="Current level progress"><span style="width:${p.percent}%"></span></div></div>
        <button class="journey-primary-action" type="button" data-journey-lesson="${esc(current.lesson.id)}">${finalMission ? "Continue to Final Boss" : p.completed ? "Continue Mission" : "Start First Mission"}<span aria-hidden="true">→</span></button>
      </div>
      <aside class="current-mission-side" aria-label="Current mission details"><b>${String(current.lessonIndex + 1).padStart(2, "0")}</b><span>${esc(current.lesson.difficulty || "Mission")}</span><small>${finalMission ? "Final challenge" : "Next step"}</small></aside>
    </section>

    <section class="current-level" aria-labelledby="current-level-title">
      <div class="journey-heading"><div><span class="journey-kicker">CURRENT LEVEL</span><h2 id="current-level-title">${esc(meta[0])}</h2><p>${esc(meta[1])}</p></div><strong>${p.completed} / ${p.total}</strong></div>
      <div class="journey-path" aria-label="Mission path for ${esc(meta[0])}"><ol>${LEVELS[currentLevel].map((_, i) => node(state, currentLevel, i, current.lessonIndex)).join("")}</ol></div>
    </section>

    <section class="nearby-levels" aria-labelledby="nearby-levels-title">
      <div class="journey-heading"><div><span class="journey-kicker">YOUR JOURNEY</span><h2 id="nearby-levels-title">Nearby levels</h2></div></div>
      <div class="journey-level-grid">${nearby}</div>
      <details class="journey-explorer"><summary><span>Explore full journey</span><small>${total.completed}/${total.total} missions completed</small></summary><div class="journey-level-grid journey-level-grid--all">${all}</div></details>
    </section>`;
}
...
function addStyles() {
  ...
  style.textContent = `
    ...
    #learning-journey{display:grid;gap:18px;padding:28px 0 42px}
