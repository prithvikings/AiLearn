...
const roadmapLevels = [
  ...
  [
    ...
    "Explore model weights, local inference and hardware.",
  ],
  [
    "Embeddings & Vector Search",
    "Turn text into numerical representations for semantic search.",
  ],
  [
    "Retrieval-Augmented Generation",
    "Connect generation to selected external knowledge.",
  ],
  ["AI Agents & Tools", "Learn systems that can choose actions and use tools."],
  [
    "Agent Memory & Planning",
    "Explore state, memory, planning, retries and longer-horizon tasks.",
  ],
  [
    "MCP",
    "Standardize how AI applications discover and interact with capabilities.",
  ],
  [
    "Agent Orchestration",
    "Coordinate multiple agents, tools, workflows, and AI components.",
  ],
  [
    "Advanced Agentic AI",
    "Explore evaluation, reliability, guardrails, observability and production AI systems.",
  ],
];
...
function render() {
  ...
  while (list.children.length < roadmapLevels.length) {
    ...
    node.innerHTML =
      '<span class="roadmap-number"></span><span class="roadmap-status"></span><h3></h3><p></p>';
  ...
  [...list.children].slice(0, roadmapLevels.length).forEach((node, index) => {
    ...
    node.querySelector(".roadmap-status").textContent = complete
      ? "✓"
      : open
        ? "●"
        : "⌑";
    node
      .querySelector(".roadmap-status")
      .setAttribute(
        "aria-label",
        complete ? "Completed" : open ? "Unlocked" : "Locked",
      );
