...
export { curriculumLevels, curriculumLevelById };
...
export function getLevelProgressById(state, levelId) { const level = getCurriculumLevel(levelId); return level ? getMissionProgress(state, level.lessons) : { completed: 0, total: 0, percent: 0, complete: false }; }
...
export function getMastery(state, levelId) { const level = getCurriculumLevel(levelId); return { levelId, title: level?.title ?? `Level ${levelId}`, ...getLevelProgressById(state, levelId) }; }
