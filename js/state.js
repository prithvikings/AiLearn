...
export function loadState() {
  try {
    ...
    const ragMissionIds=['rag-01','rag-02','rag-03','rag-04','rag-05','rag-06','rag-07','rag-08','rag-09','rag-10'];
    ...
    if (ragMissionIds.every(id=>state.completedLessons.includes(id))){unlockLevel(state,8);markLevelCompleted(state,7);}
    return state;
...
export function unlockAchievements(state,lessons){
  const unlocked=[];const foundation=lessons.filter(l=>['ai','genai','llm','apps','usecases'].includes(l.id));const llm=lessons.filter(l=>l.id.startsWith('llm-')&&l.type!=='final');const prompt=lessons.filter(l=>l.id.startsWith('prompt-'));const application=lessons.filter(l=>l.id.startsWith('app-'));const checks=[
    ...
    ['rag-rookie',state.completedLessons.includes('rag-01')],['retrieval-thinker',state.completedLessons.includes('rag-02')],['pipeline-mapper',state.completedLessons.includes('rag-03')],['knowledge-ingestor',state.completedLessons.includes('rag-04')],['retrieval-specialist',state.completedLessons.includes('rag-05')],['grounding-guardian',state.completedLessons.includes('rag-06')],['rag-debugger',state.completedLessons.includes('rag-07')],['rag-builder',state.completedLessons.includes('rag-08')],['mini-rag-engineer',state.completedLessons.includes('rag-09')],['rag-architect',state.completedLessons.includes('rag-10')]
  ];
