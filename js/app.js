...
function bindLessonInteractions() {
  ...
  if (activeLesson.type === 'attention') {
    document.querySelectorAll('[data-focus-token]').forEach((button) => button.addEventListener('click', () => { document.querySelectorAll('.attention-token').forEach((item) => item.classList.toggle('focused', item === button)); const related = button.textContent === 'cheese' ? 'cheese, famous, France' : `${button.textContent} and nearby context`; $('#attention-feedback').innerHTML = `Conceptually, the model can give different importance to relationships involving <strong>${related}</strong>.`; }));
