...
function levelTitle(levelNumber) {
  return curriculumLevels.find((level) => level.id === levelNumber)?.title || `Level ${levelNumber}`;
}
...
function animateXp(detail) {
  ...
  metric.classList.remove("celebration-xp-pop"); void metric.offsetWidth; metric.classList.add("celebration-xp-pop");
}
function pulseStreak(detail) {
  ...
  if (!streak) return;
  streak.classList.remove("celebration-pulse"); void streak.offsetWidth; streak.classList.add("celebration-pulse");
}
function animateProgress() {
  ...
  [progress, levelProgress].filter(Boolean).forEach((bar) => { bar.classList.remove("celebration-progress-fill"); void bar.offsetWidth; bar.classList.add("celebration-progress-fill"); });
}
function createConfetti(intensity = "small") {
  if (REDUCED()) return;
  const count = intensity === "major" ? 34 : 20;
  const fragment = document.createDocumentFragment(); const pieces = [];
  for (let i = 0; i < count; i += 1) {
    const piece = document.createElement("i"); piece.className = "celebration-confetti"; piece.setAttribute("aria-hidden", "true");
    piece.style.left = `${45 + Math.random() * 10}vw`; piece.style.setProperty("--dx", `${(Math.random() - .5) * (intensity === "major" ? 70 : 48)}vw`); piece.style.setProperty("--dy", `${48 + Math.random() * 50}vh`); piece.style.setProperty("--rot", `${(Math.random() - .5) * 900}deg`); piece.style.animationDelay = `${Math.random() * 100}ms`; fragment.appendChild(piece); pieces.push(piece);
  }
  document.body.appendChild(fragment);
  const timer = window.setTimeout(() => pieces.forEach((piece) => piece.remove()), 1250); animationTimers.add(timer);
}
function closeOverlay({ restore = true } = {}) {
  if (!overlay) return;
  const current = overlay; overlay = null; document.removeEventListener("keydown", onOverlayKeydown, true); current.remove();
  if (restore && restoreFocus instanceof HTMLElement && document.contains(restoreFocus)) restoreFocus.focus({ preventScroll: true });
  restoreFocus = null;
}
function onOverlayKeydown(event) {
  ...
  const focusables = [...overlay.querySelectorAll("button:not([disabled])")]; if (!focusables.length) return;
...
function showCelebration({ type, title, message, icon, rewards = [], secondary = [], confetti = "none" }) {
  closeOverlay({ restore: false }); restoreFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  overlay = document.createElement("div"); overlay.className = "celebration-overlay"; overlay.setAttribute("role", "presentation");
  overlay.innerHTML = `<section class="celebration-dialog" role="dialog" aria-modal="true" aria-labelledby="celebration-title" aria-describedby="celebration-message"><button class="celebration-dismiss" type="button" aria-label="Dismiss celebration">×</button><div class="celebration-icon" aria-hidden="true">${icon}</div><span class="celebration-eyebrow">${type}</span><h2 id="celebration-title">${title}</h2><p class="celebration-message" id="celebration-message">${message}</p>${rewards.length ? `<div class="celebration-rewards">${rewards.map(([label, value]) => `<div class="celebration-reward"><span>${label}</span><strong>${value}</strong></div>`).join("")}</div>` : ""}${secondary.length ? `<div class="celebration-secondary" aria-label="Additional rewards">${secondary.map((item) => `<div class="celebration-secondary-item">${item}</div>`).join("")}</div>` : ""}<button class="celebration-primary" type="button">Continue →</button></section>`;
  document.body.appendChild(overlay);
  const dismiss = $(".celebration-dismiss", overlay); const primary = $(".celebration-primary", overlay); const dismissAll = () => closeOverlay();
  dismiss.addEventListener("click", dismissAll); primary.addEventListener("click", dismissAll); overlay.addEventListener("click", (event) => { if (event.target === overlay) closeOverlay(); }); document.addEventListener("keydown", onOverlayKeydown, true); primary.focus({ preventScroll: true }); if (confetti !== "none") createConfetti(confetti);
}
function enhanceMissionCompletion(detail) {
  const card = $(".mission-completion-card", $("#lesson-modal")); if (!card) return;
  const mark = $(".mission-completion-mark", card); if (mark) { mark.classList.remove("celebration-success-mark"); void mark.offsetWidth; mark.classList.add("celebration-success-mark"); }
  if (detail.xpGain) { const xp = $("#mission-completion-xp", card); if (xp) { xp.textContent = `+${detail.xpGain} XP`; xp.classList.add("celebration-xp-pop"); } }
}
function celebrate(state, detail) {
  animateXp(detail); animateProgress(); pulseStreak(detail); enhanceMissionCompletion(detail);
  const hasMajor = detail.levelUp || detail.completedLevels.length > 0;
  const milestoneItems = detail.achievements.map((id) => MILESTONE_COPY[id]?.[0] || id); const secondary = [];
  ...
  if (detail.levelUp) { showCelebration({ type: "PLAYER LEVEL UP", title: `Level ${state.level}`, message: "You've reached a new player level. Keep going.", icon: "✦", rewards: [["XP earned", `+${detail.xpGain} XP`], ["Player level", `Level ${state.level}`]], secondary, confetti: "major" }); return; }
  if (detail.completedLevels.length > 0) { const level = detail.completedLevels[detail.completedLevels.length - 1]; showCelebration({ type: "LEVEL COMPLETE", title: levelTitle(level), message: `${state.completedLessons.length} missions are now complete. The next chapter is ready when you are.`, icon: "✓", rewards: [["Chapter", `Level ${level}`], ["Streak", `🔥 ${state.streak} days`]], secondary, confetti: "major" }); return; }
  if (detail.achievements.length > 0) { const [id] = detail.achievements; const [title, copy] = MILESTONE_COPY[id] || ["Milestone Unlocked", "You've reached a new milestone."]; showCelebration({ type: "MILESTONE UNLOCKED", title, message: copy, icon: "🏆", rewards: [["Milestone", title], ["Streak", `🔥 ${state.streak} days`]], secondary: secondary.filter((item) => !item.includes(title)), confetti: "small" }); return; }
  if (detail.dailyGoal) showCelebration({ type: "DAILY GOAL", title: "Goal complete", message: "Nice work. You hit today's learning goal.", icon: "✓", rewards: [["Today", `${state.dailyProgress.xp} / 100 XP`], ["Streak", `🔥 ${state.streak} days`]], secondary, confetti: "small" });
}
